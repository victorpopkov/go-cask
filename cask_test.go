	...
	for filename, expectedCask := range testCases {
		...
		if len(expectedCask.Variants) == len(actualCask.Variants) {
			for key, actualVariant := range actualCask.Variants {
				...
				assert.IsType(t, Variant{}, actualVariant)
				assert.Equal(t, expectedVariant.Version, actualVariant.Version)
				assert.Equal(t, expectedVariant.SHA256, actualVariant.SHA256)
				assert.Equal(t, expectedVariant.URL, actualVariant.URL)
				assert.Equal(t, expectedVariant.Names, actualVariant.Names)
				assert.Equal(t, expectedVariant.Homepage, actualVariant.Homepage)
				assert.Equal(t, expectedVariant.Appcast, actualVariant.Appcast)
				assert.Equal(t, expectedVariant.Artifacts, actualVariant.Artifacts)
			}
		}
	}
}
